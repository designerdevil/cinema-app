...
function readFile(path, fileName) {
    return new Promise((resolve, reject) => {
        fs.readFile(`${path}/${fileName}`, 'utf8', (err, content) => {
            if (err) {
                return reject(new Error('fail')).then(function (error) {
                    // not called
                }, function (error) {
            ...
            console.log('Error in webpack/automation/files.js readFile');
        })
    })
}
 ...
 */
async function generateFilesFromTemplate({
    name,
    path,
    templatesPath
}) {
...
function generateFiles(params) {
    ...
    const destination = `${path.toLowerCase()}/${name.toLowerCase()}`;
    const {
        testFileName,
        testFileMatch,
        componentFileName,
    ...
    if (includeTests) {
        fs.outputFile(
            `${destination}/${testFileMatch}.${testFileName}.js`,
    ...
    if (cssExtension) {
        fs.outputFile(
            ...
            generateStyleFile(styleFileName), err => {
                if (err) return console.error(err)
                /* append in AppStyle */
                const targetFile = `${defaultOptions.stylePath}/${defaultOptions.styleFile}`;
                const newFile = `${destination}/${styleFileName}`;
                appendAppStyle(targetFile, newFile);
            }
        )
    }
}
